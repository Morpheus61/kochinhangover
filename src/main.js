...
function setupEventListeners() {
    ...
    document.getElementById('downloadStatsBtn')?.addEventListener('click', downloadStatsImage);
    ...
    try {
        ...
        const cards = [
            ...
            },
            {
                ...
                valueSize: '32px',
                getValue: () => {
                    const totalRoomCount = guests.reduce((sum, guest) => {
                        const roomAmount = parseFloat(safeGetGuestProperty(guest, 'room_booking_amount', 0)) || 0;
                        return sum + Math.floor(roomAmount / 5000);  // Each room is ₹5000
                    }, 0);
                    return `₹${roomBookingRevenue} (${totalRoomCount} Rooms)`;
                }
            },
            {
                id: 'totalPax',
